> ⚠ **SUPERSEDED** — This file (010) was a duplicate number assigned before ID-stability rules were in place.
> The canonical scorecard for this niche is now at `research/scoring/004_big_feelings_anger_score.md` (niche_id: 004).
> This archive copy is retained for audit purposes only. Do not edit or use for production decisions.
> Archived: 2026-06-19

---

# Niche Scorecard 010 — Big Feelings / Anger Management / Emotional Regulation

**SUGGESTED SCORES INSIDE — Elise must confirm or override every score.**
*This scorecard was populated by Claude from Elise's manual intake file `research/_intake/004_anger_intake.md`. No Amazon data was collected by Claude. All observations are Elise's.*

---

**Niche name:** Big Feelings / Anger Management / Emotional Regulation
**Date research started:** *[Elise fills in — see intake file date_researched field]*
**Researched by:** Elise Kimi / QMN
**Scorecard file:** `research/scoring/010_big-feelings-anger_score.md`

---

## Niche Overview

- **Niche name:** Big Feelings / Anger Management / Emotional Regulation
- **Parent buyer pain point:** "My child has intense reactions — meltdowns, hitting, screaming — when frustrated. Telling them to 'calm down' doesn't work; I need something that helps them understand what's happening inside and gives them a calming tool."
- **Target age range:** 3–7 years
- **Likely Amazon search phrases** *(searched manually by Elise)*:
  1. children's book anger management
  2. big feelings picture book for kids
  3. emotional regulation kids book
  4. loud brain children's book
  5. calm brain children's book
- **QMN/MindQuest brand fit summary:** Very strong. The core MindQuest message — understanding what's happening inside before it controls you — is exactly what this niche demands. The "loud brain" framing (overwhelm before anger) connects directly to the Little Brain series and positions QMN differently from the workbook-heavy competition. High series potential as Book 002 in the Little Brain sequence.

---

## What Elise Must Check on Amazon

*From intake — already researched. Additional validation needed:*

- Confirm whether "loud brain children's book" returns any direct match with high review count — this is the primary QMN differentiation angle and must be clear
- Verify whether any new storybook-format entries (not workbooks) have appeared in the 3–7 age range with strong early traction
- Confirm the dragon metaphor is definitively owned by one competitor (Train Your Angry Dragon) and steer completely clear
- Double-check the "spot" metaphor ownership — confirm no other metaphor in the storybook space is currently unclaimed and ownable by QMN
- Note any sponsored ads at the top of "children's book anger management" — confirms commercial intent

---

## Competitor Observation Table

*Populated from Elise's manual intake. Prices not captured — fill in if observed.*

| Title | Author Type | BSR | Price | Reviews | Avg Rating | Cover Quality | Notes |
|---|---|---|---|---|---|---|---|
| Anger Management Skills Workbook for Kids (40 activities) | Individual (therapist) | #5,254 in Books | — | 1,664 | 4.7 | Mixed | Strong demand. 128 pages, therapist-positioned, functional not emotional. Not suitable for younger read-aloud ages 3–6. |
| Anger Management Workbook for Kids (50 activities) | Individual | #9,172 in Books | — | 8,977 | 4.6 | Strong | Highest review count in niche — demand proof. Activity-heavy, skews 6–10. Confirms buyers want tools, but also shows workbook format gap for 3–6. |
| I Am Stronger Than Anger | Branded series | #7,720 in Books | — | 4,584 | 4.6 | Strong | Strongest direct storybook competitor. 32-page read-aloud, proves anger storybooks sell. Directly occupies the anger-management story space — QMN must not resemble this. |
| Train Your Angry Dragon | Branded series | Not captured | — | ~5,000 | 4.7 | Strong | Series machine. Dragon metaphor owned — do not use. Confirms animal-metaphor storybooks perform. High series loyalty. |
| A Little SPOT of Anger | Branded series (56-book) | #26,352 in Books | — | 3,485 | 4.7 | Strong | Teacher's pick, concept-led series. Owns the "spot" metaphor and one calming method. Institutional buyer pattern (school/classroom). |

---

## Review Mining Notes

*Reorganized from Elise's intake notes. Elise's own words — no review text copied.*

**What parents praise most:**
A simple, repeatable calming tool combined with a story the child engages with emotionally — not just a lecture or worksheet. Parents want something that works in the moment AND is re-readable.

**What parents wish was different:**
Workbooks skew too old and feel clinical rather than like a real story. Parents of children ages 3–6 feel underserved by the activity-book format; they want something that functions as a genuine bedtime or couch read.

**Language parents use to describe the problem:**
Meltdowns, hitting, screaming, "calm down doesn't work," big feelings, mad.

**Unmet needs mentioned in reviews:**
A story-driven picture book for ages 4–7 that a child wants re-read, not a behavior-management worksheet. An upstream angle — reaching the child at the moment of overwhelm BEFORE full anger — appears underserved in the storybook format.

---

## BSR Observation Notes

- **Healthiest BSR seen in this niche:** #5,254 in Books (workbook — Anger Management Skills Workbook)
- **Most competitive title's BSR:** #7,720 in Books (I Am Stronger Than Anger, storybook)
- **General BSR range of top 10 results:** Roughly #5,000 – #26,000 in Books
- **BSR interpretation:** Strong, commercially validated demand. Multiple titles selling consistently. However, the direct "anger management" framing is crowded. The storybook sub-category has one dominant entry (#7,720) with 4,584 reviews — high bar for new storybook entrants. Workbook market is even more competitive.

---

## Market Gap Hypothesis

*Synthesized from Elise's intake gap hypothesis and competitor pattern.*

**Gap identified:** The anger management niche splits cleanly into two lanes:
1. **Workbooks (ages 6–10):** Dominated by therapist-authored activity books with thousands of reviews. Very hard to compete with directly. Ages 6+ skew.
2. **Storybooks (ages 3–6):** One very strong branded series (#7,720, 4,584 reviews) and an animal-metaphor series with ~5,000 reviews. Both own their metaphors (anger identity / dragon).

**The real gap:** Neither lane has a storybook-first book for ages 4–7 that enters through the overwhelm-before-anger doorway — the moment when the brain gets "loud" before the meltdown arrives. This upstream angle treats anger as information the brain is signaling, rather than a behavior problem to suppress. No dragon, no spot, no blob. Brain-first, overwhelm-first, storybook-first.

**Validation still required:**
- Confirm "loud brain children's book" and "calm brain children's book" return no direct match with 200+ reviews
- Confirm the overwhelming majority of existing storybook entries still rely on the anger-management framing (not the overwhelm/upstream framing)
- Validate whether any 2025–2026 titles have entered with this exact angle

---

## Series Potential

- **Book 001:** The Little Brave Brain (confidence / fear of trying new things) — *existing draft*
- **Book 002:** The Little Loud Brain (overwhelm / anger-adjacent — this niche's differentiated entry point)
- **Book 003:** The Little Calm Brain (self-regulation / finding calm)
- The series arc — brave → loud → calm — mirrors the emotional cycle parents and children recognize
- Cross-sell potential: parent buying one book becomes natural buyer for others in the series
- Year-round demand: High — anger and big feelings are perennial, not seasonal

---

## Monetization Angle

- **Likely print price range:** $10.99–$13.99 (in line with comparable storybooks)
- **eBook potential:** Yes — parents may use on tablet for in-the-moment read during or after a difficult episode
- **Companion content potential:** "My Feelings Toolbox" printable, parent co-regulation guide, "What My Brain Is Feeling" emotion wheel poster
- **Lead magnet / QMN funnel fit:** Very strong — emotional regulation is central MindQuest territory; this book is a natural funnel entry for the QMN brand

---

## Copyright / IP Risk Notes

*Preliminary from intake and existing scorecard 004 — Elise confirms during research.*

⚠️ **INSIDE OUT (Pixar/Disney) — PRIMARY IP FLAG**
Inside Out is a registered Disney/Pixar property featuring anthropomorphized emotions living inside a character's head, including a character named "Anger." Any QMN brain character must not:
- Resemble the Inside Out visual world (colorful personified emotion characters, control-room imagery)
- Use emotion-color coding similar to Inside Out (red = anger, etc.)
- Feature characters living inside a head in a way that references the Inside Out concept

**How to steer clear:** The QMN "loud brain" approach centers the brain itself as a sensory/neurological experience — not personified emotions as characters. The child IS the brain. No emotion-people inside a head. This distinction must be maintained through all illustration briefs.

**Additional IP risks:**
- "Grumpy Monkey" — trademarked; do not use monkey characters in an anger context
- Train Your Angry Dragon — dragon metaphor in anger context is owned; do not use
- A Little SPOT — spot/blob metaphor is owned; do not use
- Any specific character name must be searched on USPTO.gov before finalizing

**Overall IP risk level:** [ ] Low  [ ] **Medium-High** ✓ *(Inside Out proximity is the primary structural risk — manageable with careful character design, but cannot be treated casually)*

**Disqualifying?** No — score is not ≤ 3 if executed carefully. But character design requires deliberate IP navigation before any illustration work begins.

---

## 100-Point Scoring Table

⚠️ **SUGGESTED SCORES — Elise must confirm or override every score below.**
*Rationale for each score is tied to the rubric in `docs/niche_research_framework.md`.*

| # | Category | Max Points | Suggested Score | Notes (rationale tied to rubric) |
|---|---|---|---|---|
| 1 | Buyer urgency | 10 | **9** | "Meltdowns, hitting, screaming" = urgent, time-sensitive, crisis-level need. Rubric 9–10: urgent/time-sensitive. |
| 2 | Search demand signal | 10 | **9** | Multiple bestsellers, BSRs in #5K–#26K range, 5 competitors with 1,600–8,977 reviews. Rubric 9–10: many results, sponsored ads visible, active BSRs. |
| 3 | Competition weakness | 10 | **4** | Top storybook has 4,584 reviews; top workbook has 8,977. Branded series own their metaphors. Direct entry is hard. Rubric 3–4: strong established titles with thousands of positive reviews. Score reflects the DIRECT lane — the differentiated sub-niche (scorecards 006–009) would score higher here. |
| 4 | Parent pain point clarity | 10 | **9** | Language is extremely specific and consistent: "meltdowns, hitting, screaming, calm down doesn't work." Rubric 9–10: very specific pain point. |
| 5 | Series potential | 10 | **9** | Little Brain series (Brave → Loud → Calm) is a natural 3-book arc with cross-sell logic. Rubric 9–10: natural series. |
| 6 | Differentiation potential | 10 | **7** | The upstream/overwhelm angle (brain gets loud before anger) is genuinely differentiated. However, execution must be sharp — the metaphor space is claimed by strong competitors. Rubric 7–8: some differentiation possible with focused execution. |
| 7 | Production feasibility | 10 | **8** | 32-page storybook with original brain character — achievable. Rubric 7–8: achievable with modest creative challenge. |
| 8 | Copyright/IP safety | 10 | **5** | Inside Out (Pixar) is the primary risk for any brain/emotion-character framing. Manageable with careful design, but not low-risk. Rubric 5–6: some adjacent IP; requires careful character design and title choices. NOT disqualifying (> 3) if executed deliberately. |
| 9 | Monetization potential | 10 | **8** | Series potential + companion content (printables, parent guides) + strong gift purchase appeal. Rubric 7–8: good repeat purchase potential or upsell path. |
| 10 | QMN/MindQuest brand fit | 10 | **9** | Understanding what's happening inside before it controls you is exactly the MindQuest message. Rubric 9–10: perfect fit. |
| | **TOTAL** | **100** | **77 (suggested)** | Falls in the 60–79 range: *Revise and validate — identify weak categories before proceeding.* |

---

## Final Recommendation

**Suggested score:** 77/100 *(Elise confirms)*

**Rubric interpretation:** 60–79 = Revise and validate — identify weak categories, improve the angle before proceeding.

**Recommendation:**
[ ] **Pursue** — score 60+, brand fit confirmed, no IP red flags
[ ] **Revise** — identify weak categories and re-evaluate ← *suggested*
[ ] **Reject / Park** — score below 60 or IP risk too high

**Suggested revision path:** Do not enter through the broad "anger management" door. Enter through a differentiated sub-niche — specifically "My Brain Gets Really Loud" (Scorecard 006) or "Finding Calm / The Little Calm Brain" (Scorecard 008) — which have the same demand signal and brand fit but avoid the most crowded competition lanes. Both sub-niches should score higher on Competition Weakness if Amazon validates low competition in the upstream-overwhelm framing.

**Elise decision:** _____________________ Date: _________

*Record this score and decision in `research/gate_7_niche_shortlist.md`.*

---

*Populated from intake: `research/_intake/004_anger_intake.md` | Ref: `docs/niche_research_framework.md` | `research/manual_amazon_research_instructions.md` | `docs/copyright_policy.md` | Related sub-niche scorecards: 006, 007, 008, 009*
